...
<template>
  <div
    class="flex flex-col rounded-card border p-5"
    :class="props.current ? 'border-secondary bg-secondary/5' : 'border-border bg-surface'"
  >
    ...
    <p class="mb-4 text-2xl font-semibold text-text-primary">
      ...
      <span class="text-sm font-normal text-text-secondary">/ {{ props.plan.duration_days }} days</span>
    </p>

    <p v-if="props.plan.description" class="mb-4 flex-1 text-sm text-text-secondary">
      {{ props.plan.description }}
    </p>

    <div class="mb-4 flex items-center gap-2 text-sm text-text-secondary">
      <Check class="h-4 w-4 text-success" aria-hidden="true" />
      {{ props.plan.duration_days }}-day subscription period
    </div>
