...
<template>
  ...
  <aside
    class="fixed inset-y-0 left-0 z-40 flex w-64 flex-col bg-primary text-white transition-transform md:sticky md:top-0 md:h-screen md:translate-x-0"
    :class="props.open ? 'translate-x-0' : '-translate-x-full'"
  >
    <div class="flex h-16 items-center gap-2 px-5 text-lg font-semibold tracking-tight">
      {{ props.brandLabel }}
    </div>
