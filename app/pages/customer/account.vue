...
watchEffect(() => {
  if (profile.value) {
    ...
    phoneNumber.value = profile.value.phone_number
  }
})
...
async function handleSave() {
  ...
  try {
    ...
    saveError.value = "We couldn't save your changes. Please try again."
  } finally {
    saving.value = false
  }
}
    ...
    <form v-else class="space-y-4 rounded-card border border-border bg-surface p-5" @submit.prevent="handleSave">
      <div>
        ...
        <input
          ...
          type="email"
          disabled
          class="w-full rounded-card border border-border bg-text-secondary/5 px-3 py-2 text-sm text-text-secondary"
        />
