...
async function handleMarkRead(id: string) {
  ...
  try { await markRead(id) } catch { await refresh() }
}
</script>
<template>
  <div class="space-y-6">
    ...
    <EmptyState v-else-if="!notifications.length" :icon="Bell" title="No notifications yet" description="We'll let you know here when there's something worth your attention." />
    <ul v-else class="space-y-2">
      <li v-for="notification in notifications" :key="notification.id" class="flex items-start justify-between gap-4 rounded-card border p-4" :class="notification.is_read ? 'border-border bg-surface' : 'border-accent/30 bg-accent/5'">
        ...
        <button v-if="!notification.is_read" type="button" class="shrink-0 rounded-card border border-border px-2.5 py-1 text-xs font-medium text-text-secondary hover:bg-text-secondary/10" @click="handleMarkRead(notification.id)">Mark read</button>
      </li>
    </ul>
  </div>
</template>
