...
const { data: suggestion, pending, error, refresh } = await useAsyncData(
  `suggestion-${route.params.id}`,
  () => fetchSuggestion(route.params.id as string),
)
...
<template>
  <div class="max-w-2xl space-y-6">
    <NuxtLink to="/customer/suggestions" class="text-sm text-text-secondary hover:text-text-primary">
      ← Back to Suggestions
    </NuxtLink>
    ...
    <template v-else-if="suggestion">
      ...
      <div v-if="suggestion.admin_response" class="rounded-card border border-secondary/30 bg-secondary/5 p-5">
        ...
        <p v-if="suggestion.responded_at" class="mt-3 text-xs text-text-secondary">
