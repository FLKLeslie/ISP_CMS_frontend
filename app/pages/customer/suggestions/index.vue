...
const { data, pending, error, refresh } = await useAsyncData('customer-suggestions', () =>
  listSuggestions({ ordering: '-created_at' }),
)
...
const subject = ref('')
const category = ref<SuggestionCategory>('SUGGESTION')
const message = ref('')
...
async function handleSubmit() {
  ...
  try {
    ...
    submitError.value = "We couldn't submit this right now. Please try again."
  } finally {
...
<template>
  <div class="space-y-6">
    ...
    <EmptyState
      ...
      description="Have feedback, a complaint, or a compliment? We'd like to hear it."
    />

    <ul v-else class="space-y-2">
      <li v-for="suggestion in suggestions" :key="suggestion.id">
        <NuxtLink
          :to="`/customer/suggestions/${suggestion.id}`"
          class="flex items-center justify-between gap-4 rounded-card border border-border bg-surface p-4 transition-colors hover:border-secondary/40"
        >
          <div>
            <p class="text-sm font-medium text-text-primary">{{ suggestion.subject }}</p>
            <p class="mt-1 text-xs text-text-secondary">{{ formatRelativeTime(suggestion.created_at) }}</p>
          </div>
