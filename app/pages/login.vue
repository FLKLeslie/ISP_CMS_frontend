...
async function handleSubmit() {
  ...
  try {
    ...
    // Plain language per the design spec (§2) - never surface raw DRF
    // error shapes ("Unable to authenticate...") to the person signing in.
    ...
    <div class="w-full max-w-sm rounded-card border border-border bg-surface p-8 shadow-sm">
      ...
      <form class="space-y-4" @submit.prevent="handleSubmit">
        ...
        <button
          type="submit"
          :disabled="submitting"
          class="w-full rounded-card bg-secondary py-2.5 text-sm font-semibold text-white transition-opacity hover:opacity-90 disabled:opacity-50"
        >
